...
CREATE TABLE admin_user (
    ...
    phone_number VARCHAR(20) UNIQUE , -- 新增手机号字段及备注
...
-- 创建手机号索引并添加备注
CREATE UNIQUE INDEX idx_admin_user_phone_number ON admin_user (phone_number)
WHERE
    phone_number IS NOT NULL;

-- 插入默认系统管理员 (初始密码: admin123456，建议首次登录后修改，已使用bcrypt哈希)
INSERT INTO
    admin_user (username, email, password_hash)
VALUES
    (
        'sys_admin',
        'admin@example.com',
...
-- 分配管理员角色
INSERT INTO
    admin_roles (user_id, role_name, permissions)
VALUES
    (
        (
            SELECT
